...
from files.src.model import build_model
...
gt_df = pd.read_csv('files/input/GTSRB_Final_Test_GT/GT-final_test.csv',delimiter=';')
...
model.load_state_dict(
    torch.load(
        'files/outputs/model.pth', map_location=device
    )['model_state_dict']
)
...
def visualize_and_save_map(
    ...
    # cv2.imshow('Result', img_concat) #Docker has GUI issues
    # cv2.waitKey(10)
    if save_name is not None:
        # cv2.imwrite(f"../outputs/test_results/CAM_{save_name}.jpg", img_concat)
        cv2.imwrite(f"files/outputs/test_results/CAM_{save_name}.jpg", img_concat)
...
all_images = glob.glob('files/input/GTSRB_Final_Test_Images/GTSRB/Final_Test/Images/*.ppm')
correct_count = 0
